...
export default function Setting() {
    ...
    const uploadSqlFile = (file: Blob, callback: (result: FileReader["result"]) => void) => {
        const reader = new FileReader();
        reader.readAsText(file);
        reader.onloadend = () => callback(reader.result);
    };

    const handleUploadSqlFile = async (fileRaderResult: string | ArrayBuffer | null) => {
        const result = services?.setting({ context: fileRaderResult as string, op: opmode() });
